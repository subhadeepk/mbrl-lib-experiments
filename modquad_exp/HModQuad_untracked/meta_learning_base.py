...
class Robot:
    def __init__(self, frame_name,  client_id, motor_names=None, control_param=None):
        ...
        # if client_id==0:
        #     print("taking sent client id")
        #     self.client_id = client_id
        # # if client_id==2:
        # #     print("new client")
        # #     self.client_id = self.open_connection_box()
        # elif(frame_name=="DesiredBox"):
        #     print("for desired box")
        #     self.client_id = self.open_connection_box()
        # else:
        #     print("For robot")
    ...
    def open_connection_box(self):
        ...
        if self.client_id != -1:
            ...
            print('Connection failed', self.frame_name)
    ...
    def reset_simulation(self):
        """Reset the CoppeliaSim simulation to its starting state."""
        # Stop the simulation
        ...
        print("Simulation reset to starting state")

    def reset_positions(self, initial_pos=None, initial_ori=None, target_pos=None, target_ori=None):
        """Reset robot and target positions to initial states without restarting simulation."""
        # Default initial positions if not provided
        if initial_pos is None:
            initial_pos = [0.0, 0.0, 0.1]  # Default starting position
        if initial_ori is None:
            initial_ori = [0.0, 0.0, 0.0]  # Default starting orientation
        if target_pos is None:
            target_pos = [0.8, 0.0, 2.8]   # Default target position
        if target_ori is None:
            target_ori = [0.0, 0.0, 0.0]   # Default target orientation
            
        # Reset robot position and orientation
        self.set_position(initial_pos)
        self.set_orientation(initial_ori)
        ...
        print(f"Robot reset to position {initial_pos}, orientation {initial_ori}")
        return initial_pos, initial_ori, target_pos, target_ori
    ...
    def set_orientation(self, orientation, relative_object=-1):
        ...
        if err_code != 0:
            print("ERROR: CANNOT SET {} ORIENTATION W.R.T. {} TO {}".format(self.frame_name, relative_object, orientation))
...
def get_state(r1, d1): 
    """
    state for each robot is 12 dimensional right now, cart and angular
